...
static void print_usage(const char *prog) {
    fprintf(stderr,
        ...
        "  --debug       : print corrected data to stdout\n"
...
static int calibration_mode(struct bmi270 *sensor) {
    ...
    // Step 1
    printf("Step 1: place flat, press Enter (timeout %d s): ", CAL_TIMEOUT_SEC); fflush(stdout);
    ...
    // Step 2
    printf("Step 2: tilt ~45°, press Enter (timeout %d s): ", CAL_TIMEOUT_SEC); fflush(stdout);
    FD_ZERO(&rfds); FD_SET(STDIN_FILENO,&rfds);
    tv.tv_sec=CAL_TIMEOUT_SEC; tv.tv_usec=0;
    if (select(STDIN_FILENO+1,&rfds,NULL,NULL,&tv)<=0) { fprintf(stderr,"\nTimeout.\n"); return -1; }
    fgets(buf,sizeof(buf),stdin);
    ...
    vec3 tilt={sum_tilt.x/N, sum_tilt.y/N, sum_tilt.z/N};
    printf("Tilt mean [%.3f %.3f %.3f]\n", tilt.x,tilt.y,tilt.z);
    if (save_calibration(&flat,&tilt)<0) { perror("Saving calibration"); return -1; }
...
int main(int argc,char *argv[]) {
    ...
    if(do_cal){ return calibration_mode(&sensor)<0?1:0; }
    load_calibration();

    int sock=-1; struct sockaddr_in addr;
    if(do_udp){ sock=socket(AF_INET,SOCK_DGRAM,0); if(sock<0){perror("socket");return 1;} memset(&addr,0,sizeof(addr));addr.sin_family=AF_INET;addr.sin_port=htons(UDP_PORT); if(inet_pton(AF_INET,UDP_IP,&addr.sin_addr)!=1){fprintf(stderr,"Invalid UDP_IP '%s'\n",UDP_IP);return 1;} }

    FILE *fp=NULL;
    if(do_log){ time_t now=time(NULL); char path[256]; snprintf(path,sizeof(path),"/tmp/%s_%ld.gcsv",LOG_ID,now); fp=fopen(path,"w"); if(!fp){perror("fopen");return 1;} fprintf(fp,"%s\nversion,1.3\nid,%s\norientation,%s\n", HEADER_TYPE,LOG_ID,ORIENTATION); fprintf(fp,"note,%s\nfwversion,%s\ntimestamp,%ld\nvendor,%s\n", NOTE,FWVERSION,(long)now,VENDOR); fprintf(fp,"videofilename,%s\nlensprofile,%s\nlens_info,%s\n", VIDEO_FILENAME,LENS_PROFILE,LENS_INFO); fprintf(fp,"frame_readout_time,%.2f\nframe_readout_direction,%d\n", FRAME_READOUT_TIME,FRAME_READOUT_DIR); fprintf(fp,"tscale,%.6f\nascale,%.8f\ngscale,%.8f\n", TSCALE,ASCALE,GSCALE); fprintf(fp,"t,gx,gy,gz,ax,ay,az\n"); }
    ...
    while(1){
        ...
        uint32_t dt=(uint32_t)(((curr.tv_sec-prev.tv_sec)*1e3)+((curr.tv_nsec-prev.tv_nsec)/1e6)); prev=curr;
        if(do_debug) printf("%u | dt=%u ms | ax=%.3f ay=%.3f az=%.3f gx=%.3f gy=%.3f gz=%.3f\n",count,dt,aa[0],aa[1],aa[2],ag[0],ag[1],ag[2]);
        if(do_udp){ int32_t buf[NUM_FIELDS]; buf[0]=count; for(int i=0;i<3;i++) buf[1+i]=(int32_t)(ag[i]/GSCALE); for(int i=0;i<3;i++) buf[4+i]=(int32_t)(aa[i]/ASCALE); sendto(sock,buf,sizeof(buf),0,(struct sockaddr*)&addr,sizeof(addr)); }
        if(do_log){ fprintf(fp,"%u,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",count,ag[0],ag[1],ag[2],aa[0],aa[1],aa[2]); fflush(fp); }
        count++;
