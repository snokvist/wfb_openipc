...
def calculate_alink_link_health(video_rx):
    """
    Recalculate ALINK link health from JSON/msgpack data using hard-coded parameters.
    """
...
def send_radio_status_alink(conn, recovered_packet_count):
    """
    Construct and send a MAVLink RADIO_STATUS message based on ALINK link health data.
    Uses the local recovered_packet_count for both fixed and rxerrors.
    Also logs the sent message via a helper function (which the main module will handle).
    """
    ...
    # Optionally, you could log this message via a callback or similar.
    # For example, if the main module provides an 'add_to_alink_sent_history' function.
    
def run_alink_thread(conn, shutdown_event):
    """
    Connect to the ALINK JSON/msgpack server, fetch Wi-Fi link data,
    recalculate link health, and send a RADIO_STATUS message via the given MAVLink connection.
    """
