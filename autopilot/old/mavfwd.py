...
#########################################
# STATUSTEXT File Check Function
#########################################

def check_and_send_status_text(conn):
    """
    Checks every 500ms for the file /tmp/message.mavlink. If found,
    reads its content, sends it as a STATUSTEXT message (severity 6) over the provided MAVLink connection,
    and deletes the file.
    """
    filename = "/tmp/message.mavlink"
    if os.path.exists(filename):
        try:
            with open(filename, "r") as f:
                text = f.read().strip()
            if text:
                # Use severity 6 (informational); limit text to 50 characters.
                severity = 6
                text = text[:50]
                conn.mav.statustext_send(severity, text)
                print(f"Sent STATUSTEXT: {text}")
            os.remove(filename)
        except Exception as e:
            print(f"Error handling STATUSTEXT file: {e}")
...
def run_sync_mode(args):
    ...
    Sync mode:
      ...
      - Also checks every 500ms for /tmp/message.mavlink to send a STATUSTEXT.
    """
    ...
    # Create incoming connection on forced port 14550.
    ...
    while not shutdown_event.is_set():
        ...
        # Check for STATUSTEXT file every 500ms.
        if now - last_file_check >= 0.5:
            check_and_send_status_text(hb_conn)
            last_file_check = now
